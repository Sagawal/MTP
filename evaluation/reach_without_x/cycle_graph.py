...
def log(Q_list, res, answer, args):
    ...
    newpath = 'log/reach_cycle/'+args.model+'-'+args.mode+'-'+time+'-'+args.prompt
    if args.SC == 1:
        newpath = newpath + "+SC"
...
def main():
    ...
    for i in tqdm(range(25,30)):
        with open("generation/cycle_graphs/"+args.mode+"/cycle_graph_"+str(i)+".txt","r") as f, open("answers/output2_cycle_medium.txt","a", encoding="utf-8") as f5:

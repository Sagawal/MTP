...
def log(Q_list, res, answer, args):
    ...
    newpath = 'log/reach_random/'+args.model+'-'+args.mode+'-'+time+'-'+args.prompt
    if args.SC == 1:
        newpath = newpath + "+SC"
...
def main():
    ...
    for i in tqdm(range(20,40)):
        with open("generation/random_graphs/"+args.mode+"/random_graph_"+str(i)+".txt","r") as f, open("answers/output2_random_medium.txt","a", encoding="utf-8") as f5:

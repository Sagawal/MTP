...
def translate(m, edges, queries, args):
    ...
    Q_list = []
    for i in range(10):
        # Q_i = Q + "node "+str(queries[i][0])+" and node "+str(queries[i][1])+" without going through node "+str(queries[i][2])+"?\nA:"
        Q_i = Q + "node "+str(queries[i][0])+" and node "+str(queries[i][1])+" without going through node "+str(queries[i][2])+"? \nStep by step reasoning is: First check for all possible paths from node "+str(queries[i][0])+" to node "+str(queries[i][1])+". Then exclude any paths that pass through node " + str(queries[i][2])+ "If at least one path remains then return answer as '\nA:Yes, there is a path.' and if not then return '\nA:No, there is no path.' "
        Q_list.append(Q_i)
    return Q_list
...
def log(Q_list, res, answer, args):
    ...
    newpath = 'log/reach_not_through_xx/'+args.model+'-'+args.mode+'-'+time+'-'+args.prompt
    if args.SC == 1:
        newpath = newpath + "+SC"
...
def main():
    ...
    for i in tqdm(range(25,50)):
        with open("generation/path_graphs/"+args.mode+"/path_graph_"+str(i)+".txt","r") as f, open("answers/output2_path_hard_zero_cot.txt","a", encoding="utf-8") as f5:
            ...
            for j in range(5):
                ...
                for k in range(sc):
                    ...
                    # if "yes" in ans:
                    if "yes, there is a path" in ans or "yes. there is a path" in ans or "yes there is a path" in ans:
                        vote += 1
                if vote * 2 >= sc:
                    res.append(1)
                else:
            ...
            for j in range(5):
                ...
                for k in range(sc):
                    ...
                    f5.write("i value: " + str(i)+" and j+5 value: "+str(j)+ans+"\n")
                    answer.append(ans)
                    #ans = os.linesep.join([s for s in ans.splitlines() if s]).replace(' ', '')
                    # if "no" in ans:
                    if "no, there is no path" in ans or "no. there is no path" in ans or "no there is no path" in ans:
                        vote += 1
                if vote * 2 >= sc:
                    res.append(1)
                else:
